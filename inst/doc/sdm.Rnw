...
\subsection{Importing occurence data}
...
The "wrld\_simpl" dataset contains rough country outlines. You can use other datasets of polygons (or lines or points) as well. For example, you can read a shapfile into \R using the \texttt{readOGR} function in the \texttt{rgdal} package or the \texttt{readShapePoly} function in the \texttt{maptools} package.
...
\chapter{Model fitting, prediction, and evaluation}
...
To speed up processing, let's retstruct the predictions to a more restricuted area (defined by a rectangular extent):
<<sdm32, echo=TRUE>>=
...
Earlier on, we fitted a Bioclim model using data.frame with each row representing the environmental data at known sites of presence of a species. Here we fit a bioclim model simly using the predictors, and the occurence points.
...
\subsection{time: climate change}
