...
Here is an example for one of the records with longitude = 0, using Google's geocoding service. We put the function into a 'try' function, to assure elegant error handling if the computer is not connected to the Internet.

<<sdm9>>=
args(biogeomancer)
# the first locality:
loc <- paste(lonzero$locality[1], ", Peru")
loc
b <- try(  geocode(loc, oneRecord=T) )
b
@
